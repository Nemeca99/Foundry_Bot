...
class SystemDashboard:
    ...
    def get_system_health(self) -> Dict:
        ...
        return {
            ...
            "warnings": warnings,
            "healthy_components": len([c for c in health_checks.values() if c["status"] == "healthy"])
        }
    ...
    def get_component_status(self) -> Dict:
        ...
        return {
            ...
            "image_generation": {
                "status": "operational" if self.check_image_generation()["status"] == "healthy" else "limited",
                "models_available": len(self.image_generator.get_available_styles()),
                "cuda_available": self.image_generator.get_model_status().get("cuda_available", False)
            },
            "voice_generation": {
                "status": "operational" if self.check_voice_generation()["status"] == "healthy" else "limited",
                "engines_available": len(self.voice_generator.get_available_voices()["voice_presets"]),
                "pyttsx3_available": self.voice_generator.get_engine_status().get("pyttsx3", False)
            },
            "video_generation": {
                "status": "operational" if self.check_video_generation()["status"] == "healthy" else "limited",
                "styles_available": len(self.video_generator.get_available_styles()),
                "moviepy_available": self.video_generator.get_engine_status().get("moviepy", False)
            },
            "audio_processing": {
                "status": "operational" if self.check_audio_processing()["status"] == "healthy" else "limited",
            ...
            "emotional_systems": {
                "status": "operational" if self.check_emotional_health()["status"] == "healthy" else "limited",
                "blender_operational": True,
                "engine_operational": True
            },
            "multimodal_orchestration": {
                "status": "operational" if self.check_orchestration()["status"] == "healthy" else "limited",
                "capabilities_available": len([c for c in self.orchestrator.get_available_capabilities().values() if c])
            }
        }
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if resource_health["status"] == "warning":
            recommendations.append("Consider optimizing system resources - high CPU or memory usage detected")
        elif resource_health["status"] == "critical":
            recommendations.append("URGENT: System resources critically low - consider restart or optimization")
        
        # Check component health
        multimodal_health = self.check_multimodal_health()
        if multimodal_health["status"] == "warning":
            recommendations.append("Some multimodal systems have limited functionality - check API keys and model availability")
        elif multimodal_health["status"] == "critical":
            recommendations.append("CRITICAL: Multimodal systems failing - check model installations and API configurations")
        
        # Check emotional systems
        emotional_health = self.check_emotional_health()
        if emotional_health["status"] != "healthy":
            recommendations.append("Emotional systems need attention - check emotional fragment files and psychological models")
        
        # General recommendations
        if not recommendations:
            recommendations.append("System is operating optimally - no immediate action required")
        ...
        for metric, value in overview['performance_metrics'].items():
            if isinstance(value, dict):
                ...
                for sub_metric, sub_value in value.items():
                    report += f"  • {sub_metric.replace('_', ' ').title()}: {sub_value}\n"
            else:
    ...
    def monitor_system(self, interval: int = 60, duration: int = None):
        ...
        try:
            while True:
                ...
                overview = self.get_system_overview()
                print(f"Overall Health: {overview['system_health']['overall_status'].upper()}")
                print(f"Critical Issues: {overview['system_health']['critical_issues']}")
                print(f"Warnings: {overview['system_health']['warnings']}")
                print(f"Healthy Components: {overview['system_health']['healthy_components']}")
                
                # Show resource usage
                resources = overview['resource_usage']
                print(f"CPU: {resources['cpu']['usage_percent']}% | Memory: {resources['memory']['usage_percent']}% | Disk: {resources['disk']['usage_percent']}%")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="AI Writing Companion System Dashboard")
    parser.add_argument("--report", action="store_true", help="Generate and display system report")
