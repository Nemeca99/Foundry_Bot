...
class CharacterDevelopmentEngine:
        ...
        try:
            ...
            for character, arc_data in self.character_arcs.items():
                if isinstance(arc_data, dict):
                    ...
                    arc_dict['development_triggers'] = [trigger.value for trigger in arc_data.development_triggers]
            ...
            for event in self.development_events:
                if isinstance(event, dict):
                    ...
                    if 'trigger' in event_copy and hasattr(event_copy['trigger'], 'value'):
    ...
    def create_character_arc(self, character_name: str, arc_description: str,
                           initial_stage: DevelopmentStage = DevelopmentStage.INTRODUCTION) -> CharacterArc:
        ...
        arc_dict['development_triggers'] = [trigger.value for trigger in arc.development_triggers]
    ...
    def add_development_event(self, character_name: str, trigger: DevelopmentTrigger,
                            description: str, impact_score: float = 0.5,
                            growth_areas_affected: List[str] = None,
                            before_state: Dict[str, Any] = None,
                            after_state: Dict[str, Any] = None) -> DevelopmentEvent:
    ...
    def _update_character_arc_from_event(self, character_name: str, event: DevelopmentEvent):
    ...
    def _update_character_growth(self, character_name: str, event: DevelopmentEvent):
        ...
        growth_data['development_history'].append(f"{event.timestamp}: {event.description}")
        
        # Update transformation level
        total_growth = sum(growth_data['growth_areas'].values())
        growth_data['transformation_level'] = min(total_growth / 10.0, 1.0)  # Normalize to 0-1
    ...
    def get_development_events(self, character_name: str, 
                             trigger: DevelopmentTrigger = None) -> List[Dict]:
    ...
    def suggest_character_development(self, character_name: str) -> List[str]:
        ...
        if not arc_data:
            suggestions.append(f"Create a character arc for {character_name} to track their development journey")
            return suggestions
        ...
        if current_stage == 'introduction':
            suggestions.append(f"Introduce {character_name} to a significant challenge or conflict")
            suggestions.append(f"Establish {character_name}'s initial goals and motivations")
        elif current_stage == 'conflict':
            suggestions.append(f"Create opportunities for {character_name} to face their fears")
            suggestions.append(f"Introduce obstacles that test {character_name}'s core beliefs")
        elif current_stage == 'growth':
            suggestions.append(f"Show {character_name} learning from their experiences")
            suggestions.append(f"Have {character_name} develop new skills or perspectives")
        elif current_stage == 'crisis':
            suggestions.append(f"Present {character_name} with their biggest challenge yet")
            suggestions.append(f"Force {character_name} to question their fundamental assumptions")
        elif current_stage == 'resolution':
            suggestions.append(f"Allow {character_name} to overcome their final obstacle")
            suggestions.append(f"Show {character_name} achieving their primary goal")
        elif current_stage == 'transformation':
            suggestions.append(f"Demonstrate {character_name}'s complete transformation")
            suggestions.append(f"Show how {character_name} has changed the world around them")
        ...
        if growth_data:
            ...
            if growth_areas:
                ...
                suggestions.append(f"Focus on developing {character_name}'s {weakest_area[0]} (current level: {weakest_area[1]:.2f})")
    ...
    def analyze_character_progress(self, character_name: str) -> Dict[str, Any]:
        ...
        average_impact = sum(e.get('impact_score', 0) for e in events) / total_events if total_events > 0 else 0
        
        # Analyze growth areas
        growth_areas = growth_data.get('growth_areas', {})
        strongest_area = max(growth_areas.items(), key=lambda x: x[1]) if growth_areas else ("none", 0)
        weakest_area = min(growth_areas.items(), key=lambda x: x[1]) if growth_areas else ("none", 0)
    ...
    def get_character_development_summary(self, character_name: str) -> str:
        ...
        summary_parts.append(f"- Current stage: {arc_data.get('current_stage', 'unknown')}")
        summary_parts.append(f"- Total development events: {len(events)}")
        summary_parts.append(f"- Transformation level: {growth_data.get('transformation_level', 0.0):.2f}")
    ...
    def create_development_scenario(self, character_name: str, scenario_type: str) -> Dict[str, Any]:
        """Create a development scenario for a character"""
        arc_data = self.get_character_arc(character_name)
        current_stage = arc_data.get('current_stage', 'introduction') if arc_data else 'introduction'
    ...
    def advance_character_manually(self, character_name: str, new_stage: DevelopmentStage):
    ...
    def reset_character_development(self, character_name: str):
        ...
        self.development_events = [
            event for event in self.development_events
            if event.get('character_name') != character_name
        ]
