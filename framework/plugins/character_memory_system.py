...
class CharacterMemorySystem:
    ...
    def add_memory(self, character_name: str, memory_type: MemoryType, content: str,
    ...
    def _update_knowledge_base(self, character_name: str, memory: Memory):
        ...
        if memory.memory_type == MemoryType.KNOWLEDGE_MEMORY:
            ...
            knowledge_keywords = re.findall(r'\b(know|learn|understand|discover|find)\b', 
            ...
            skill_keywords = re.findall(r'\b(skill|ability|talent|capability|expertise)\b', 
                                      memory.content, re.IGNORECASE)
            if skill_keywords:
                skill_name = memory.content.split()[0] if memory.content.split() else "unknown_skill"
    ...
    def _update_relationships_from_memory(self, character_name: str, memory: Memory):
        ...
        for other_character in memory.associated_characters:
            ...
            relationship['relationship_history'].append(f"{memory.timestamp}: {memory.content[:50]}...")
            ...
            relationship['strength'] = min(1.0, relationship['strength'] + 
                                        importance_multiplier[memory.importance])
    
    def get_character_memories(self, character_name: str, memory_type: MemoryType = None) -> List[Memory]:
    ...
    def get_character_relationships(self, character_name: str) -> List[Dict[str, Any]]:
        ...
        for relationship_key, relationship_data in self.relationships.items():
            if (relationship_data['character_a'] == character_name or 
                relationship_data['character_b'] == character_name):
    ...
    def add_backstory(self, character_name: str, backstory_content: str, 
                     backstory_type: str = "general") -> bool:
    ...
    def search_memories(self, character_name: str, query: str) -> List[Memory]:
        ...
        for memory in memories:
            if (query_lower in memory.content.lower() or
                query_lower in memory.character_name.lower() or
                any(query_lower in char.lower() for char in memory.associated_characters)):
    ...
    def get_relationship_summary(self, character_a: str, character_b: str) -> str:
        ...
        relationship_key = f"{min(character_a, character_b)}_{max(character_a, character_b)}"
        ...
        summary_parts.append(f"- Relationship type: {relationship['relationship_type']}")
        ...
        summary_parts.append(f"- Shared experiences: {len(relationship['shared_experiences'])}")
        
        if relationship['relationship_history']:
            summary_parts.append("- Recent interactions:")
            for interaction in relationship['relationship_history'][-3:]:  # Last 3 interactions
    ...
    def reset_character_memories(self, character_name: str):
        ...
        for key in self.relationships:
            if (self.relationships[key]['character_a'] == character_name or 
                self.relationships[key]['character_b'] == character_name):
