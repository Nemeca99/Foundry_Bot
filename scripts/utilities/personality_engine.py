...
Your voice is unified and balanced, harmonizing all aspects into one cohesive expression."""
    ...
    def get_blended_voice_description(self, emotional_state: EmotionalState) -> str:
        ...
        if max_weight > 0.8:
            ...
            blend_description = self._create_harmonious_blend(
                frag1, voice1, weight1, frag2, voice2, weight2
            )
            ...
            if primary_fragments:
                ...
                if secondary_fragments:
                    secondary_influences = [
                        self._get_voice_influence(f, 0.3)
                        for f in secondary_fragments[:2]
                    ]
                    return (
                        f"{primary_voice} enhanced by {', '.join(secondary_influences)}"
                    )
                else:
                ...
                equal_voices = [
                    self.get_voice_description(f) for f, _ in fragment_voices[:3]
                ]
    ...
    def get_context_aware_voice(
        self, emotional_state: EmotionalState, context: str = ""
    ) -> str:
        ...
        if any(
            word in context_lower for word in ["technical", "logic", "analysis", "data"]
        ):
            return (
                f"{base_voice} - Enhanced with technical precision and analytical focus"
            )
        elif any(
            word in context_lower for word in ["emotional", "feeling", "heart", "care"]
        ):
            return (
                f"{base_voice} - Enhanced with emotional depth and caring sensitivity"
            )
        elif any(
            word in context_lower
            for word in ["creative", "imagination", "art", "possibility"]
        ):
            return f"{base_voice} - Enhanced with creative expression and imaginative thinking"
        elif any(
            word in context_lower for word in ["memory", "past", "history", "remember"]
        ):
            return f"{base_voice} - Enhanced with memory integration and historical context"
        elif any(
            word in context_lower
            for word in ["protection", "security", "safety", "defense"]
        ):
            return f"{base_voice} - Enhanced with protective instincts and security awareness"
        elif any(
            word in context_lower
            for word in ["passion", "desire", "intimacy", "connection"]
        ):
