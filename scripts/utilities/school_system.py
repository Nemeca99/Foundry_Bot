...
class SchoolSystem:
    ...
    def get_or_create_player(self, player_id: str) -> PlayerEducation:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "SELECT * FROM player_education WHERE player_id = ?", (player_id,)
            )
            ...
            if result:
                ...
                # Load school levels
                cursor.execute(
                    "SELECT * FROM school_levels WHERE player_id = ?", (player_id,)
                )
    ...
    def attend_school(
        self, player_id: str, school_type: SchoolType, minutes: int = 1
    ) -> Dict:
    ...
    def answer_trivia(
        self, player_id: str, school_type: SchoolType, answer: str
    ) -> Dict:
    ...
    def get_player_education_summary(self, player_id: str) -> Dict:
        ...
        for school_type, school_level in player.schools.items():
            ...
            school_summaries[school_type.value] = {
                ...
                "success_rate_bonus": self.get_success_rate_bonus(
                    player_id, school_type
                ),
    ...
    def _get_specialization(self, player: PlayerEducation) -> str:
        ...
        max_schools = [
            school_type
            for school_type, school in player.schools.items()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    player.total_level,
                    player.retired,
                    player.retirement_date,
                    player.retirement_rank,
                    player.comeback_attempts,
                    player.player_id,
                ),
            )
            ...
            for school_type, school_level in player.schools.items():
                ...
                """,
                    (
                        school_level.level,
                        school_level.learning_points,
                        school_level.last_activity,
                        school_level.decay_start,
                        school_level.total_time_spent,
                        school_level.trivia_answered,
                        school_level.bonus_points_earned,
                        player.player_id,
                        school_type.value,
                    ),
                )
