#!/usr/bin/env python3
"""
Automated Stress Test v2 for Simulacra Rancher
Enhanced version with better error handling and more realistic scenarios
"""
...
class AutoStressTesterV2:
    """Enhanced automated stress tester with better error handling"""
    ...
    def create_test_users(self, num_users: int = 10):
        ...
        for i in range(num_users):
            ...
            user = {
                "user_id": user_id,
                "username": username,
                "rp": random.randint(50, 200),  # Random starting RP
                "resources": {},
                "memories": [],
                "hunting_history": [],
                "daily_claimed": False,
        ...
        print(f"✅ Created {num_users} test users with random RP")
        return self.users
                    ...
                    if user["rp"] >= rp_cost:
                        try:
                            ...
                            if "database is locked" in str(e):
                                ...
                                result["success"] = False
                                result["response"] = f"❌ Hunt error: {str(e)}"
                    else:
                        result["success"] = True
                        result["response"] = (
    ...
    def generate_realistic_command(self, user: Dict) -> tuple:
        """Generate realistic commands based on user behavior patterns"""
        commands = []

        # Daily bonus (only if not claimed)
        if not user.get("daily_claimed", False):
            commands.append(("daily", []))

        # Resource gathering (40% chance)
        ...
        # Hunting (30% chance if user has RP)
        if random.random() < 0.3 and user["rp"] > 20:
        ...
        # Personality interaction (15% chance)
        ...
        # Leaderboard check (20% chance)
        if random.random() < 0.2:
            commands.append(("leaderboard", []))

        # Kingdom check (15% chance)
        if random.random() < 0.15:
            commands.append(("kingdoms", []))
    ...
    def should_continue_testing(self) -> bool:
        ...
        if self.stats["start_time"]:
            ...
            if elapsed.total_seconds() > 7200:  # 2 hours max
                print("⏰ Time limit reached (2 hours)")
                return False
    ...
    def run_continuous_stress_test(self, max_iterations: int = 50000):
        """Run continuous stress test until resources are exhausted"""
        print("🚀 Starting Enhanced Continuous Stress Test")
        print("=" * 60)
        ...
        try:
            while self.running and iteration < max_iterations:
                ...
                # Generate realistic command
                command, args = self.generate_realistic_command(user)
                ...
                if result["rp_change"] != 0 or not result["success"]:
                    print(
                        f"📝 [{iteration:05d}] {user['username']} > {command} {' '.join(args)}"
                    )
                    print(f"     {result['response']}")

                # Print status every 200 iterations
                if iteration % 200 == 0:
                    self.print_status()
                ...
                time.sleep(0.005)  # Faster execution
    ...
    def print_final_results(self):
        ...
        print(f"\n💾 Results saved to: stress_test_results_v2.json")

    def save_results(self):
        """Save test results to file with proper JSON serialization"""
        results = {
            "timestamp": datetime.now().isoformat(),
            "stats": {
        ...
        with open("stress_test_results_v2.json", "w") as f:
            json.dump(results, f, indent=2)


def main():
    """Main function to run the enhanced stress test"""
    print("🔥 SIMULACRA RANCHER STRESS TEST v2")
    print("=" * 60)
    print("Enhanced version with better error handling and realistic scenarios")
    print("This will run continuously until all users run out of RP")
    print("Press Ctrl+C to stop early")
    print("=" * 60)

    tester = AutoStressTesterV2()
