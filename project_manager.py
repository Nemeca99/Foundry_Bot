...
class ProjectManager:
    ...
    def __init__(self, data_directory: str = "data/projects"):
        ...
        self.writing_assistant = self.framework.get_plugin("writing_assistant")
        self.personality_engine = self.framework.get_plugin("personality_engine")
    ...
    def create_project(self, title: str, description: str, genre: str, 
                      target_audience: str, word_count_goal: int,
                      priority: ProjectPriority = ProjectPriority.MEDIUM,
                      due_date: str = None, tags: List[str] = None) -> Project:
    ...
    def search_projects(self, query: str) -> List[Project]:
        ...
        for project in self.projects.values():
            if (query in project.title.lower() or 
                ...
                query in project.genre.lower() or
                any(query in tag.lower() for tag in project.tags)):
                results.append(project)
        
        return results
    
    def add_chapter(self, project_id: str, title: str, content: str = "") -> Optional[Chapter]:
        ...
        for chapter in project.chapters:
            if chapter['id'] == chapter_id:
                ...
                if 'content' in kwargs:
                    ...
                    project.current_word_count = project.current_word_count - old_word_count + chapter['word_count']
    ...
    def add_character(self, project_id: str, name: str, description: str,
                     personality: str, role: str) -> Optional[Character]:
    ...
    def get_project_statistics(self, project_id: str) -> Dict:
        ...
        completed_chapters = len([c for c in project.chapters if c.get('status') == 'completed'])
        total_characters = len(project.characters)
        
        progress_percentage = (project.current_word_count / project.word_count_goal * 100) if project.word_count_goal > 0 else 0
        
        days_created = (datetime.now() - datetime.fromisoformat(project.created_date)).days
    ...
    def generate_ai_suggestions(self, project_id: str) -> Dict:
        ...
        try:
            ...
            if project.chapters:
                ...
                plot_suggestions = self.writing_assistant.generate_plot(project.title, plot_prompt)
            ...
            if project.characters:
                ...
                character_suggestions = self.writing_assistant.develop_character(project.title, "main_character", character_prompt)
                suggestions["character_development"].append(character_suggestions)
            
            # Generate writing improvements
            if project.chapters:
                latest_chapter = max(project.chapters, key=lambda x: x.get('last_modified', ''))
                if latest_chapter.get('content'):
                    improvement_prompt = f"Suggest improvements for this writing: {latest_chapter['content'][:500]}..."
                    improvements = self.writing_assistant.rewrite(project.title, improvement_prompt)
            ...
            next_steps = self.writing_assistant.brainstorm("next_steps", project.genre, next_steps_prompt)
    ...
    def backup_projects(self) -> str:
        """Backup all projects"""
        backup_file = self.data_directory / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument("--create", nargs=6, metavar=("TITLE", "DESCRIPTION", "GENRE", "AUDIENCE", "WORD_COUNT", "PRIORITY"), help="Create a new project")
    ...
    parser.add_argument("--suggestions", type=str, help="Generate AI suggestions for project")
    parser.add_argument("--export", nargs=2, metavar=("PROJECT_ID", "FORMAT"), help="Export project")
    ...
    if args.list:
        ...
        for project in projects:
            ...
            print(f"  • {project.title} ({project.genre}) - {stats['progress_percentage']}% complete")
