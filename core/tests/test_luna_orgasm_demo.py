...
def interactive_orgasm_demo():
    ...
    while True:
        try:
            ...
            print(f"\nLuna: {response}")
            print(
                f"Emotional Level: {status['current_level']:.1f} - {status['current_state']}"
            )
...
def test_specific_orgasm_triggers():
    ...
    framework.update_emotional_state(
        "You're so sexy and beautiful, I want you so badly"
    )
    ...
    for trigger in orgasm_triggers:
        ...
        status = framework.get_emotional_status()
        print(
            f"Current Level: {status['current_level']:.1f} - {status['current_state']}"
        )
